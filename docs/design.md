...
- **机场搜索热度**:记录每个机场被搜索/查看的次数,后台按机场展示热度排行(可按时间窗看趋势),用于判断哪些机场最受关注、指导数据维护优先级
- 巴士信息维护:树形选国家/城市/机场 → 编辑线路(保存即触发推送闭环)
- 操作记录:audit_log 列表 + 过滤

> **机场搜索热度(功能补充)**:查询机场时累加该机场的搜索计数(异步、不阻塞查询;高频写入用 Redis 计数 + 周期落库,避免每次搜索都打 DB)。计数维度至少到机场粒度;后台读榜单。这是只增的统计数据,与 audit_log(操作追责)分开。
